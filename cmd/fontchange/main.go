...
func createControls(hwnd uintptr) {
	title := label(hwnd, "L4D2 Font_change 字体切换工具", 24, 24, 560, 34)
	procSendMessageW.Call(title, wmSetFont, titleFont, 1)
	desc := label(hwnd, "此修改字体方法在游戏使用 L4N 平台时无效；L4N 平台字体请参考 https://www.bilibili.com/video/BV1JpCjB9Ewp/\r\n选择系统已安装字体或先安装自备字体；确认字体后再更换，恢复默认会删除游戏目录内同名文件。", 24, 68, 860, 54)
	...
	appendLog("[prepare] ready; font source: resources\\Font_change")
	setStatus("正在加载系统字体...")
}
...
func readFontModName() string {
	...
	data, err := os.ReadFile(filepath.Join(fontDir, "FontMod.yaml"))
	if err != nil {
		return ""
	}
	...
	for _, line := range lines {
		if strings.TrimSpace(line) == "Tahoma:" {
			inTahoma = true
			continue
		}
		if inTahoma {
